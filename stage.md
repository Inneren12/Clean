...
- **Lifecycle/DI:** FastAPI lifespan now wires a typed `AppServices` container (storage, email, Stripe, rate limiting, metrics) onto `app.state.services` with shutdown cleanup to reduce global-state bugs.
