...
class Settings(BaseSettings):
    ...
    auth_access_token_ttl_minutes: int = Field(
        15, env=["AUTH_ACCESS_TOKEN_TTL_MINUTES", "AUTH_TOKEN_TTL_MINUTES"]
    )
    auth_refresh_token_ttl_minutes: int = Field(60 * 24 * 14, env="AUTH_REFRESH_TOKEN_TTL_MINUTES")
    auth_session_ttl_minutes: int = Field(60 * 24, env="AUTH_SESSION_TTL_MINUTES")
    password_hash_scheme: Literal["argon2id", "bcrypt"] = Field("argon2id", env="PASSWORD_HASH_SCHEME")
    password_hash_argon2_time_cost: int = Field(3, env="PASSWORD_HASH_ARGON2_TIME_COST")
    password_hash_argon2_memory_cost: int = Field(65536, env="PASSWORD_HASH_ARGON2_MEMORY_COST")
    password_hash_argon2_parallelism: int = Field(2, env="PASSWORD_HASH_ARGON2_PARALLELISM")
    password_hash_bcrypt_cost: int = Field(12, env="PASSWORD_HASH_BCRYPT_COST")
    session_ttl_minutes_worker: int = Field(60 * 12, env="SESSION_TTL_MINUTES_WORKER")
    session_ttl_minutes_client: int = Field(60 * 24 * 7, env="SESSION_TTL_MINUTES_CLIENT")
    session_rotation_grace_minutes: int = Field(5, env="SESSION_ROTATION_GRACE_MINUTES")
