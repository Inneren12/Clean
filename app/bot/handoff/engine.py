...
def _fsm_is_progressing(fsm_reply: FsmReply) -> bool:
    if fsm_reply.progress is not None:
        return True

    if fsm_reply.quick_replies:
        return True

    return fsm_reply.step in FLOW_STEPS
...
def evaluate_handoff(
    ...
    fsm_progressing = _fsm_is_progressing(fsm_reply)
    has_letters = any(ch.isalpha() for ch in message_text)
    ...
    if fsm_progressing and intent_result.intent != Intent.faq and has_letters:
    ...
    if intent_result.intent == Intent.faq and faq_matches is not None and not faq_matches:
        if not has_letters:
            return HandoffDecision(
                ...
                suggested_action="Clarify the request manually and confirm next steps.",
                summary=_format_summary(fsm_reply.summary),
            )

        if "fallback to faq" in intent_result.reasons or intent_result.confidence <= 0.25:
            return HandoffDecision(
                should_handoff=True,
                reason="faq_unclear",
    ...
    if intent_result.confidence < HANDOFF_CONFIDENCE_THRESHOLD and not faq_matches:
        if (
            (not fsm_progressing or not has_letters)
            and (is_routing_step or intent_result.intent == Intent.faq)
            and not _is_short_or_greeting(message_text)
            and (
                _is_long_complex(message_text)
                or intent_result.confidence < LOW_CONFIDENCE_DEEP_THRESHOLD
                or not has_letters
            )
        ):
            return HandoffDecision(
